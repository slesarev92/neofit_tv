...
### Раунд 2: оценка пути решения и приоритеты гипотез

**Что было правильным:**
- Начали с «железных» причин (fps/битрейт/профиль/level) — это медиа-инженерная диагностика, не «вебовая»
- Level 3.1 для 1080p — реально могло давать софтварный fallback
- Переход от blob.slice() к Blob URL — логичный шаг как гипотеза

**Что пошло не туда:**
- Blob URL для онлайн-режима — «не в ту сторону» для микроплавности на H616/2GB
- toBlobUrl() = resp.blob() = большая разовая загрузка в RAM → GC-паузы
- Умножено на preload следующего ролика → второй blob параллельно
- **Ключевой инсайт:** на Android-приставке плавность чаще ломает НЕ сам `<video>`, а фоновые задачи вокруг него (кэширование/пересчёт размеров/предзагрузка/аллокации памяти)

**Ранжирование гипотез по вероятности (от внешнего эксперта):**
1. **Очень вероятно:** (7) enforceLimit() и/или (8) aggressive precache создают фоновые пики CPU/IO/памяти
2. **Очень вероятно:** (6)+(9) toBlobUrl() (особенно на больших mp4) даёт RAM/GC эффект
3. **Возможно:** (10) аппаратный декод не включается стабильно (после фикса профиля/level шанс ниже, нужны метрики dropped frames)

**Итоговый вывод:** blob URL — не базовый режим для онлайна, это аварийный offline fallback. Телеметрия (`071dd8b`) — правильный следующий шаг: покажет blobTimeMs/canplayTimeMs, корреляцию с poll/PRECACHE, и droppedFrames (софт vs hw).
