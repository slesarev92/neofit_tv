...
**Текущая версия:** v2.3 (package.json: 2.3.0)
...
| v2.0 | Исправление кэша логотипа в сайдбаре: в `updateSidebarBrand()` добавлен cache-buster (`?t=Date.now()`), чтобы при открытии/обновлении страницы Настроек не показывалась старая картинка. |
| v2.1 | Переключатель тёмной темы: привязка в nav.js после вставки переключателя. Логотип: без `src` в HTML (нет мелькания), применение по кнопке «Сохранить», превью по blob-URL. Кнопка «Сохранить» в блоке «Система» сохраняет название и логотип. Бэкап: в архив добавлены файлы логотипа и заставки «вне часов»; в настройках — пояснение «резервное копирование настроек». |
| v2.2 | Бэкап: при «Сделать бэкап» — ввод имени архива (модальное окно); кнопка «Загрузить бэкап» — список архивов и восстановление из выбранного. Отображение названий бэкапов в списке (стили и fallback). |
| v2.3 | Плейлисты: дублирование элементов — один медиафайл можно добавить в плейлист несколько раз. Кнопка «Дублировать» (⧉) на каждом элементе, вставка копии сразу после оригинала. Идентификация по id элемента; бейдж «×2», «×3» для повторяющегося медиа. |
