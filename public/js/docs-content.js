  ...
  window.DOCS_SECTIONS = [
    ...
    },
    {
      ...
      content: '<h4>Зачем нужен APK</h4>' +
        '<p>Нативное приложение обеспечивает автозапуск при включении питания, киоск-режим (блокировка кнопки «Назад»), удержание экрана включённым (FLAG_KEEP_SCREEN_ON) и привязку к экрану по коду/QR. Собрано с <code>android:largeHeap="true"</code> для увеличенного heap на устройствах с 2 ГБ RAM.</p>' +
        '<h4>Установка</h4>' +
        '<p>Скачайте <code>app-debug.apk</code> из админки (ссылка «Загрузить APK» в меню; требуется авторизация) и установите на приставке через файловый менеджер. Рекомендуется устанавливать как обычное приложение (не в системный раздел).</p>' +
    ...
    },
    {
      ...
      content: '<p>Ключевые файлы и папки бэкенда и фронтенда:</p>' +
        ...
        '<li><code>upload-apk.ps1</code> — PowerShell‑скрипт загрузки <code>app-debug.apk</code> на сервер по SCP.</li>' +
        '</ul>'
    },
  ];
