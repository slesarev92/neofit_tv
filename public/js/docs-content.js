  ...
  window.DOCS_SECTIONS = [
    ...
    },
    {
      ...
      content: '<h4>Зачем нужен APK</h4>' +
        ...
        '<p>Чтобы попасть в настройки (URL сервера, ID экрана, смена PIN) из работающего плеера, нужно <strong>удерживать палец на экране 5 секунд</strong>. Появится запрос PIN — введите текущий PIN и нажмите OK. После этого откроется экран настроек.</p>' +
        '<ul>' +
        '<li><strong>PIN по умолчанию</strong>: <code>1234</code>. Хранится только на устройстве (SharedPreferences), на сервер не передаётся.</li>' +
        '<li><strong>При первом запуске</strong> после ввода правильного PIN приложение потребует сменить PIN (минимум 4 цифры). Отменить смену нельзя — это защита от доступа с заводским кодом.</li>' +
        ...
        '<li><strong>Если PIN забыт</strong>: можно открыть настройки через ADB (<code>adb shell am start -n com.signage.player/.SettingsActivity</code>) или удалить данные приложения / переустановить APK (после этого PIN снова станет 1234 и при первом входе снова попросят сменить).</li>' +
        '</ul>'
    },
    {
      id: 'pairing',
