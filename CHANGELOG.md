...
### Fixed
- **Оффлайн-boot всё ещё ловил статический «Нет контента / Плейлист не назначен», даже после native playlist-кэша.** Корневая причина оказалась глубже всех предыдущих фиксов: `/js/player.js` отдавался Express'ом **без `Cache-Control`** (в `server.js::express.static` setHeaders ранний `return` для не-HTML), а Service Worker этот путь не перехватывал — кэшировались только `/uploads/*` и `/api/player/*`. WebView полагался на эвристическую свежесть Chromium (~10% от `now − Last-Modified`) — после force-stop / reboot / эвикции под памятью скрипт пропадал из HTTP-кэша. WebView грузил `index.html` (там `max-age=3600` стоял), но `player.js` падал в network → сети нет → скрипт не выполнился → исходный HTML-плейсхолдер «Нет контента» застывал навсегда. Все три предыдущих fallback'а (SW-кэш API, localStorage, native SharedPreferences) бесполезны без `player.js`. Каскад правок: (a) `server.js::express.static` теперь выставляет `Cache-Control: public, max-age=86400` для `.js/.css/.png/.svg/.ico/.woff*`, `no-cache` для `sw.js` (чтобы апдейты доходили), HTML-логика прежняя; (b) `public/sw.js`: новый `SHELL_CACHE = 'signage-shell-v1'` отдельно от `CACHE_NAME` (media), `install` делает `cache.addAll(['/player/index.html', '/js/player.js', '/favicon.png'])`, fetch-handler добавил ветку stale-while-revalidate для shell-путей; (c) `public/sw.js::precacheUrls` пропускает `/api/player/*` и shell-пути в eviction-loop — раньше успешный poll **удалял** закэшированный API-ответ из media-кэша, потому что URL API не входил в `currentFullSet` (там только URL'ы из плейлиста). После деплоя: первый онлайн-визит установит обновлённый SW, тот пре-кэширует shell; следующий offline-boot отдаст shell из SHELL_CACHE → `player.js` запустится → каскад fallback'ов (native cache → localStorage → SW API-cache) сработает → плейлист подхватится → ExoPlayer SimpleCache воспроизведёт видео. (2026-05-17)
...
- **Оффлайн-boot всё ещё показывал «Плейлист не назначен» после SW-фикса.** Зависимость только от SW оказалась хрупкой: на конкретной приставке (Allwinner H616 WebView) кэш SW после `destroy()` не выживал между запусками приложения. Добавлен независимый fallback в `public/js/player.js`: каждый успешный `poll()` теперь сохраняет полный ответ `/api/player/...` в `localStorage` под ключом `lastPlaylist_<screenId>`; при network/SW-failure poll читает оттуда и продолжает рендерить. Также `waitForSwController` timeout сокращён 1500ms → 300ms (раньше блокировал первый poll в WebView, где SW недоступен или claim не fires). Дополнительно: сразу после запуска player.js подменяет текст placeholder на «Загрузка плейлиста…» — это маркер того, что скрипт реально стартовал. Если на оффлайн-boot пользователь по-прежнему видит исходный «Плейлист не назначен», значит /js/player.js не загрузился (cache miss в WebView HTTP cache) и проблема не в SW. (2026-05-16)
...
### Added
- **Phase B мульти-брендинга — Gradle product flavors.** Один и тот же исходник теперь собирается в три отдельных APK с разными `applicationId` и `app_name` в launcher: `app-neofit-debug.apk` (NeoFit TV, `com.signage.player` — без суффикса, чтобы существующие установки апгрейдились в плейс), `app-labgym-debug.apk` (Labgym TV, `com.signage.player.labgym`) и `app-soham-debug.apk` (Soham TV, `com.signage.player.soham`). На устройстве в launcher теперь видно правильный бренд клуба, а не общее «NeoFit TV». Каждый flavor также инжектирует `BuildConfig.DEFAULT_SERVER_URL`, и Spinner на первом запуске преселектит соответствующий клуб (Labgym APK → выбран «Labgym TV», и т. д.). `server.js::GET /app-debug.apk` теперь brand-aware: подбирает файл по `settings.systemName`, fallback на legacy `app-debug.apk` для плавной миграции. **Внимание:** на устройствах Labgym/Soham, где уже стоит старый APK c `com.signage.player`, новый brand-APK установится side-by-side; старый нужно вручную удалить после проверки нового. Затронуты: `android-app/app/build.gradle` (productFlavors + buildConfigField), `strings.xml` (убран `app_name`, теперь из flavor.resValue), `BindingActivity.kt` + `SettingsActivity.kt` (BuildConfig-aware default), `server.js`, `scripts/upload-apk.ps1` (новый параметр `-Flavor`), `docs/DEPLOYMENT.md`, `CLAUDE.md`. (2026-05-16)
